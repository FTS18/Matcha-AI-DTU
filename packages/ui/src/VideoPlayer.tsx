...
function kMeans2(samples: number[][]): [number[], number[]] {
  ...
  let c1 = [...samples[0]];
  let c2 = [...samples.reduce((best, c) => (colorDist(c, c1) > colorDist(best, c1) ? c : best), samples[1])];
  for (let iter = 0; iter < 12; iter++) {
    const g1: number[][] = [],
      g2: number[][] = [];
    for (const c of samples) (colorDist(c, c1) <= colorDist(c, c2) ? g1 : g2).push(c);
    if (!g1.length || !g2.length) break;
    const mean = (g: number[][]) => g[0].map((_, i) => Math.round(g.reduce((s, c) => s + c[i], 0) / g.length));
    c1 = mean(g1);
    c2 = mean(g2);
  }
  return [c1, c2];
...
function sampleJersey(
  ...
  try {
    ...
    ctx.drawImage(video, bx + bw * 0.2, by + bh * 0.25, sw, sh, 0, 0, 1, 1);
...
export const VideoPlayer = forwardRef<VideoPlayerRef, VideoPlayerProps>(
  ({ src, events, highlights, onTimeUpdate, seekFnRef, initialTeamColors, trackingData, adOverlayUrl }, ref) => {
    ...
    // Clips with videoUrl
    const clips = useMemo(() => highlights.filter((h) => h.videoUrl), [highlights]);
    ...
    const drawLoop = useCallback(() => {
      ...
      if (showTracking && video.readyState >= 2) {
        ...
        if (trackingData && trackingData.length > 0) {
          ...
          while (trackIdx.current > 0 && (trackingData[trackIdx.current]?.t ?? 0) > video.currentTime) {
            trackIdx.current--;
          }
    ...
    const handleTimeUpdate = useCallback(() => {
      ...
      if (clipMode.type === "main") {
        for (const ev of events) {
          if (
            !seenEvents.current.has(ev.id) &&
            ev.timestamp >= v.currentTime - 0.8 &&
            ev.timestamp <= v.currentTime + 0.8
          ) {
      ...
      >
        ...
        <div className="relative bg-black w-full h-full flex items-center justify-center">
          <video
            ...
            onEnded={() => {
              setPlaying(false);
              if (clipMode.type === "clip") nextClip();
            }}
            onLoadedMetadata={() => {
          ...
          <AnimatePresence>
            {showControls && (
              ...
              >
                ...
                <div className="flex gap-2 pointer-events-auto">
                  <div
                    className={`flex items-center gap-1.5 text-[11px] font-bold uppercase px-3 py-1.5 rounded-full backdrop-blur-md shadow-lg transition-colors
 ${modelState === "ready" ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : modelState === "loading" ? "bg-zinc-800/80 text-zinc-400 animate-pulse border border-zinc-600/30" : "bg-red-500/10 text-red-400 border border-red-500/20"}`}
                  ...
                  >
                    <span
                      className={`size-1.5 rounded-full ${showTracking ? "bg-primary shadow-[0_0_8px_rgba(255,255,255,0.8)]" : "bg-zinc-500"}`}
                    />
 ...
 ${clipMode.type === "main" ? "bg-primary/10 text-primary" : "text-white/70 hover:bg-white/5 hover:text-white"}`}
                >
 ...
 ${isActive ? "bg-primary/5 text-primary" : "text-white/80 hover:bg-white/5 hover:text-white"}`}
                      >
                        <div
                          className={`size-8 rounded-full flex items-center justify-center shrink-0 font-black text-xs
