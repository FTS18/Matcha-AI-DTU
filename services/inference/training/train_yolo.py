...
def train_custom_yolo(data_yaml, model_size="s", epochs=50, imgsz=640):
    ...
    # Train the model
    print(f" Starting training with {epochs} epochs on {data_yaml}...")
    results = model.train(
        data=data_yaml,
        epochs=epochs,
        imgsz=imgsz,
    ...
    if results and results.save_dir:
        print(
            f" Training complete! Best weights saved to: {results.save_dir}/weights/best.pt"
        )
        print(
            " Copy these weights to services/inference/ and update analysis.py to use them."
        )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--size", type=str, default="s", help="Model size (n, s, m, l, x)"
    )
