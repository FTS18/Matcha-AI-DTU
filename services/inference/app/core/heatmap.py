...
DST_POLY = np.array(
    [[0, 0], [PITCH_W, 0], [PITCH_W, PITCH_H], [0, PITCH_H]], dtype=np.float32
)
...
def generate_heatmap(
    track_frames: list, output_path: str, team_colors_rgb: Optional[List] = None
) -> bool:
    """
    ...
    for grid, color in [(grid_a, col_a), (grid_b, col_b)]:
        ...
        mask = (
            np.stack([alpha_ch, alpha_ch, alpha_ch], axis=-1).astype(np.float32) / 255.0
        )
        canvas = (
            canvas.astype(np.float32) * (1 - mask)
            + heat_color.astype(np.float32) * mask
        ).astype(np.uint8)
...
def _draw_legend(canvas: np.ndarray, col_a: tuple, col_b: tuple) -> None:
    ...
    for i, (label, color) in enumerate(labels):
        ...
        cv2.rectangle(
            canvas, (pad, y - box_size + 4), (pad + box_size, y + 4), color, -1
        )
        cv2.putText(
            canvas,
            label,
