...
def _check_supervision():
    ...
    if _supervision_available is None:
        try:
            ...
            logger.warning(
                "supervision not installed — soccer analysis overlay disabled"
            )
...
def _check_sklearn():
    ...
    if _sklearn_available is None:
        try:
            ...
            logger.warning(
                "scikit-learn not installed — team colour clustering disabled"
            )
...
def process_clip_frames(
    ...
    *before* FFmpeg encoding.
    ...
    try:
        ...
        return (
            frames  # Return original frames on failure — never break the highlight reel
        )
