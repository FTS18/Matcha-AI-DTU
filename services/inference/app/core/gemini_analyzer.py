...
def _parse_gemini_events(response_text: str, duration: float) -> List[Dict]:
    ...
    for item in data:
        ...
        final_score = round(
            min(10.0, base_score * confidence + (1 - confidence) * 3), 1
        )
        ...
        highlights.append(
            {
                ...
                "endTime": (
                    min(duration, ev["timestamp"] + 5.0)
                    if duration > 0
                    else ev["timestamp"] + 5.0
                ),
                "score": ev["finalScore"],
                "eventType": ev["type"],
...
async def analyze_video_with_gemini(
    ...
    try:
        ...
        if video_path.startswith("http://localhost") or video_path.startswith(
            "http://127.0.0.1"
        ):
            # Extract filename from URL like http://localhost:4000/uploads/filename.mp4
            filename = (
                video_path.split("/uploads/")[-1] if "/uploads/" in video_path else None
            )
            if filename:
        ...
        if duration <= 0:
            ...
            logger.warning(
                f"Could not get duration via ffprobe, estimated: {duration:.1f}s"
            )
...
If no sports events are detected, respond with: []"""
        ...
        for i, event in enumerate(events):
            ...
            logger.info(
                f"Sent event: {event['type']} @ {event['timestamp']}s (conf: {event['confidence']})"
            )
...
Respond ONLY in {language} language."""
        ...
        logger.info(
            f"Analysis complete for match {match_id}: {len(events)} events, {len(highlights)} highlights (HTTP {resp.status_code})"
        )
