    ...
    if _event_detector is None:
        try:
            ...
            if model_path.exists():
                ...
                logger.info(
                    "SoccerNet temporal detector initialized (no pre-trained weights)"
                )
...
def detect_events_from_features(
    ...
    for peak_idx in peaks:
        ...
        post_peak = feature_diffs[
            peak_idx : min(peak_idx + int(sample_fps * 8), len(feature_diffs))
        ]
...
def detect_football_events(video_path: str, sensitivity: float = 1.0) -> List[Dict]:
    ...
    events = detect_events_from_features(
        features, fps, duration, sample_fps=2.0, sensitivity=sensitivity
    )

    logger.info(f"SoccerNet detected {len(events)} events:")
    for ev in events:
        logger.info(
            f" {ev['type']} at {ev['timestamp']:.1f}s (conf={ev['confidence']:.2f})"
        )
