...
class Cfg:
    ...
    DEFAULT_GOAL_POLY_NORM = np.array(
        [
            [0.00, 0.30],
            [0.08, 0.30],
            [0.08, 0.70],
    ...
    DEFAULT_GOAL_POLY_NORM_RIGHT = np.array(
        [
            [0.92, 0.30],
            [1.00, 0.30],
            [1.00, 0.70],
...
class KalmanBallTracker:
    ...
    def _init_track(obs: BallObs) -> TrackState:
        ...
        kf.measurementNoiseCov = (
            np.eye(2, dtype=np.float32) * 2e-1
        )  # Increased from 1e-1 for smoothing
...
class CameraCutDetector:
    ...
    def is_cut(self, frame: np.ndarray) -> bool:
        ...
        hist = (
            cv2.calcHist([gray], [0], None, [64], [0, 256]).flatten().astype(np.float32)
        )
...
class GoalGeometry:
    ...
    def auto_calibrate(self, frame: Optional[np.ndarray] = None):
        ...
        if frame is not None:
            ...
            if len(lines) > 2:
                ...
                logger.info(
                    f"GoalGeometry: detected {len(lines)} pitch lines for calibration"
                )
    ...
    def point_in_goal(self, cx: float, cy: float) -> bool:
        ...
        if self._use_topdown and self._homography is not None:
            ...
            r = cv2.pointPolygonTest(
                self._topdown_poly, (float(td[0]), float(td[1])), False
            )
        else:
    ...
    def goal_center(self) -> Tuple[float, float]:
        if self._poly_px is not None:
            return float(np.mean(self._poly_px[:, 0])), float(
                np.mean(self._poly_px[:, 1])
            )
    ...
    def detect(self, frame: np.ndarray, frame_id: int) -> Optional[BallObs]:
        ...
        try:
            ...
            prediction = self._model.predict(
                frame, confidence=int(Cfg.YOLO_CONF * 100)
            ).json()
            ...
            if in_goal:
                ...
                if self._dwell >= Cfg.DWELL_FRAMES:
                    ...
                    if toward or self._dwell >= Cfg.DWELL_FRAMES + 2:
                        ...
                        conf = min(
                            ball_conf
                            + min((self._dwell - Cfg.DWELL_FRAMES) * 0.05, 0.15)
                            + min(speed / 60.0, 0.10),
                            0.99,
                        )
                        ...
                        direction = (
                            "left_goal"
                            if gcx < self._geo.fw * 0.4
                            else "right_goal" if gcx > self._geo.fw * 0.6 else "center"
                        )
                        logger.info(
                            f"GOAL at t={timestamp:.2f}s dwell={self._dwell} speed={speed:.1f} conf={conf:.3f}"
                        )
                        return GoalEvent(
                            frame=frame_id,
                            timestamp=timestamp,
...
class GoalDetectionEngine:
    ...
    def __init__(
        self, model_path: str = "yolov8n.pt", roboflow_cfg: Optional[dict] = None
    ):
        self._detector = None
    ...
    def init(self, frame_w: int, frame_h: int, fps: float):
        ...
        self._geometry_right.set_goal_polygon(
            Cfg.DEFAULT_GOAL_POLY_NORM_RIGHT, build_homography=True
        )
    ...
    def process_frame(self, frame: np.ndarray) -> Optional[GoalEvent]:
        ...
        if w > 960:
            scale = 960.0 / w
            proc = cv2.resize(
                frame, (960, int(h * scale)), interpolation=cv2.INTER_LINEAR
            )
        else:
    ...
    def process_video(
        self, video_path: str, frame_step: int = 2, callback=None
    ) -> List[GoalEvent]:
