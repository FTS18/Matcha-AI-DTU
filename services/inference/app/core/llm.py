...
def _call_gemini_with_retry(
    model, content, max_retries: int = 2, timeout: float = 30.0
):
    """
    ...
    with _GEMINI_SEMAPHORE:
        for attempt in range(max_retries + 1):
            try:
                ...
                logger.warning(
                    f"Gemini call timed out after {timeout}s (attempt {attempt+1}/{max_retries+1})"
                )
                if attempt < max_retries:
                    continue
                raise TimeoutError(
                    f"Gemini call timed out after {max_retries+1} attempts"
                )
            except Exception as e:
                err = str(e)
                is_not_found = (
                ...
                if is_not_found:
                    logger.warning(
                        f"Gemini model not found/deprecated, resetting model cache: {err[:120]}"
                    )
                    with _model_lock:
                        _gemini_model = None
                    raise
                ...
                if is_rate:
                    ...
                    if is_daily:
                        logger.warning(
                            " Gemini DAILY quota exhausted — disabling Gemini for this session"
                        )
                    ...
                    if attempt < max_retries:
                        ...
                        logger.warning(
                            f"Gemini rate limited (attempt {attempt+1}/{max_retries+1}), retrying in {wait:.1f}s…"
                        )
                        time.sleep(wait)
                        continue
                raise
...
def analyze_frames_batch(frames_with_ts: List[Tuple[np.ndarray, float]]) -> List[dict]:
    ...
    _none = {
        "event_type": "NONE",
        "confidence": 0.0,
        "description": "Vision AI unavailable",
    }
    ...
    if not uncached:
        logger.debug(
            f"Vision cache: all {len(frames_with_ts)} frames served from cache"
        )
    ...
    try:
        ...
        for pos, i in enumerate(uncached, 1):
            res = parsed.get(
                pos,
                {"event_type": "NONE", "confidence": 0.0, "description": "parse-error"},
            )
            results[i] = res
...
def analyze_frame_with_vision(
    frame: np.ndarray, timestamp: float, context: str = ""
) -> dict:
...
def generate_commentary(
    ...
    energy = (
        "HIGH INTENSITY"
        if final_score >= 7.5
        else ("MODERATE" if final_score >= 5 else "low key")
    )
    late_str = (
        "in the dying minutes (CRUCIAL late-game moment!)"
        if late
        else f"at minute {minute}"
    )
    ctx_str = ""
    if context_events:
...
def generate_commentary_parallel(
    scored_events: list,
    duration: float,
    ...
    if _quota_exhausted:
        logger.info(
            "Commentary: Gemini quota exhausted — using template fallback for all events"
        )
    ...
    def _one(args):
        ...
        return i, generate_commentary(
            ev["type"], ev["finalScore"], ev["timestamp"], duration, ctx, language
        )
