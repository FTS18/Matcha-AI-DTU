...
def _annotate_clip_with_soccer_analysis(
    ...
    try:
        ...
        if (
            subprocess.run(
                re_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE
            ).returncode
            == 0
        ):
...
def create_highlight_reel(
    ...
    next_idx = (
        1 + (1 if has_crowd else 0) + (1 if has_roar else 0) + 1  # music
    )  # +1 for the video itself at index 0
...
def precompress_video(video_path: str, match_id: str, config: dict) -> str:
    try:
        if (
            ...
            < config["COMPRESS_SIZE_THRESHOLD_MB"]
        ):
            return video_path
        ...
        cmd = [
            ...
            video_path,
            "-vf",
            f"scale=-2:{config['COMPRESS_OUTPUT_HEIGHT']},fps={config['COMPRESS_OUTPUT_FPS']}",
            "-c:v",
            "libx264",
            "-crf",
            "28",
        ...
        if (
            subprocess.run(cmd, timeout=config["FFMPEG_COMPRESS_TIMEOUT"]).returncode
            == 0
        ):
