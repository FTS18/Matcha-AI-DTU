...
class GoalpostDetector:
        ...
        try:
            ...
            for contour in contours:
                ...
                confidence = fill_ratio * (
                    min(aspect_ratio / self.max_aspect_ratio, 1.0)
                )
    ...
    def draw_on_frame(
        self, frame: np.ndarray, goalpost_pair: GoalpostPair
    ) -> np.ndarray:
        """
...
class GoalpostTracker:
    ...
    def update(self, detection: Optional[GoalpostPair]) -> Optional[GoalpostPair]:
        ...
        smoothed = GoalpostPair(
            ...
            center_x=alpha * detection.center_x
            + (1 - alpha) * self.last_detection.center_x,
            center_y=alpha * detection.center_y
            + (1 - alpha) * self.last_detection.center_y,
            goal_width=alpha * detection.goal_width
            + (1 - alpha) * self.last_detection.goal_width,
