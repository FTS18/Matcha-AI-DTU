...
class PhysicsDetector:
    ...
    def process_frame(self, frame_data: dict):
            ...
            if avg_v > 0.5 and velocity < 0.1:  # Ball was fast, suddenly stopped
                ...
                for p in players:
                    if _distance(curr_pos, _get_player_centroid(p)) < 0.05:
                        is_goal_area = (
                            curr_pos[0] < GOAL_ZONE_LEFT
                            or curr_pos[0] > GOAL_ZONE_RIGHT
                        )
                        ...
                        self.events.append(
                            {
                                ...
                                "confidence": 0.75,
                                "source": "cv_physics",
                                "desc": f"Ball deceleration block ({ev_type})",
                            }
                        )
        ...
        if curr_pos[0] < GOAL_ZONE_LEFT or curr_pos[0] > GOAL_ZONE_RIGHT:
            ...
            if self.goal_zone_dwell_frames == int(
                self.fps * 1.0
            ):  # 1s dwell in goal zone
                ...
                if self.last_ball_pos is not None:
                    if (
                        curr_pos[0] < GOAL_ZONE_LEFT
                        and self.last_ball_pos[0] > curr_pos[0]
                    ):
                        was_moving_toward_goal = True
                    elif (
                        curr_pos[0] > GOAL_ZONE_RIGHT
                        and self.last_ball_pos[0] < curr_pos[0]
                    ):
                        was_moving_toward_goal = True

                if was_moving_toward_goal or self.goal_zone_dwell_frames >= int(
                    self.fps * 2.0
                ):
                    self.events.append(
                        {
                            ...
                            "confidence": 0.85,
                            "source": "cv_physics",
                            "desc": "Ball dwelled in goal zone",
                        }
                    )
                else:
                    self.goal_zone_dwell_frames = 0
        ...
        if self.last_ball_pos and len(self.ball_velocities) >= 3:
            ...
            if velocity > 0.4 and avg_v < 0.2:  # Sudden high acceleration
                ...
                towards_goal = (moving_left and curr_pos[0] < 0.5) or (
                    not moving_left and curr_pos[0] > 0.5
                )
                    ...
                    for p in players:
                        if len(p) >= 42:  # has pose keypoints (8 base + 34 kps)
                            ...
                            if (
                                lax > 0
                                and lay > 0
                                and _distance(self.last_ball_pos, (lax, lay)) < 0.08
                            ) or (
                                rax > 0
                                and ray > 0
                                and _distance(self.last_ball_pos, (rax, ray)) < 0.08
                            ):
                                self.events.append(
                                    {
                                        "timestamp": t,
                                        ...
                                        "confidence": 0.85,
                                        "source": "cv_physics",
                                        "desc": "Shot from player foot detected",
                                    }
                                )
                                break
                    ...
                    self.events.append(
                        {
                            "timestamp": t - 0.5,
                            "type": "TACKLE",
                            "confidence": min(0.4 + overlap, 0.8),
                            "source": "cv_physics",
                            "desc": "Player collision/tackle",
                        }
                    )
        ...
        if len(players) >= 5:
            ...
            if (
                max_cluster_size >= 6 and cluster_center is not None
            ):  # 6+ players in tight cluster
                # Check location
                        ...
                        self.events.append(
                            {
                                ...
                                "confidence": 0.8,
                                "source": "cv_physics",
                                "desc": "Player cluster at corner flag",
                            }
                        )
                elif (
                    cluster_center[0] < 0.25 or cluster_center[0] > 0.75
                ) and 0.3 < cluster_center[1] < 0.7:
                    # In box/penalty arc - could be free kick or celebration
                    # Check recent events for a goal
                    recent_goal = any(
                        e["type"] == "GOAL"
                        for e in self.events
                        if t - e["timestamp"] < 15.0
                    )
                    if recent_goal:
                        self.events.append(
                            {
                        ...
                        self.events.append(
                            {
                                ...
                                "confidence": 0.6,
                                "source": "cv_physics",
                                "desc": "Player cluster (free kick setup)",
                            }
                        )
        ...
        if balls and players:
            ...
            if 0.45 < curr_pos[0] < 0.55 and 0.45 < curr_pos[1] < 0.55:
                ...
                near_players = sum(
                    1
                    for p in players
                    if _distance(curr_pos, _get_player_centroid(p)) < 0.15
                )

                # Check team separation
                team_0_xs = [
                    _get_player_centroid(p)[0]
                    for p in players
                        ...
                        self.events.append(
                            {
                                ...
                                "confidence": 0.9,
                                "source": "cv_physics",
                                "desc": "Start of Game / Kickoff detected",
                            }
                        )
    ...
    def get_deduplicated_events(self) -> List[Dict]:
        ...
        for ev in sorted_evs:
            ...
            last_ev = merged[-1]
            if (
                ev["type"] == last_ev["type"]
                and (ev["timestamp"] - last_ev["timestamp"]) < 5.0
            ):
