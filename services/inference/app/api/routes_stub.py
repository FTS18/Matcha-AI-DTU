...
def _generate_emotion_scores(duration: float, interval: float = 5.0):
    ...
    while t <= duration:
        ...
        audio = max(
            0.0, min(1.0, base_audio + noise_a + 0.2 * abs(random.gauss(0, 0.3)))
        )
        motion = max(
            0.0, min(1.0, base_motion + noise_m + 0.2 * abs(random.gauss(0, 0.3)))
        )
...
async def analyze_video_stub(
    ...
    try:
        ...
        for i, event in enumerate(mock_events):
            ...
            logger.info(
                f"[Stub] Sent event {event['type']} @ {event['timestamp']}s ({progress}%)"
            )
...
async def analyze_match(
    ...
    logger.info(
        f"Received analysis request for match {request.match_id} (video: {request.video_url})"
    )
    ...
    return {
        ...
        "mode": "gemini" if GEMINI_AVAILABLE else "stub",
    }
