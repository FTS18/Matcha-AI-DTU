...
export default function Home() {
  ...
  useEffect(() => {
    if (user) {
      client
        .getStats()
        .then((data: { totalMatches: number; totalEvents: number; totalHighlights: number; totalDuration: number }) => {
    ...
    <div className="relative w-full overflow-hidden">
      ...
      <div className="relative min-h-[calc(100vh-80px)] flex flex-col items-center justify-center border-b border-border py-16">
        ...
        <div className="relative z-10 max-w-360 mx-auto px-6 sm:px-8 w-full flex flex-col items-center text-center gap-6 sm:gap-10">
          ...
          >
            Upload raw match footage. The AI pipeline detects goals, fouls, saves &amp; tackles — scores each moment —
            and builds highlight reels with live neural commentary.
          </p>
          ...
          >
            ...
            {[
              ...
              <div
                key={s.label}
                className="text-center px-4 sm:px-0 sm:pr-16 last:pr-0 border-r last:border-r-0 border-border/40 relative z-10 flex-1 sm:flex-none"
              >
                <div className="font-display leading-none text-[28px] sm:text-[54px] text-primary drop-shadow-[0_0_12px_rgba(var(--color-primary),0.5)]">
                  {s.n}
                </div>
                <div className="font-mono mt-1 sm:mt-2 text-[7px] sm:text-[10.5px] text-muted-foreground uppercase tracking-[0.16em] font-bold">
                  {s.label}
                </div>
              </div>
            ))}
